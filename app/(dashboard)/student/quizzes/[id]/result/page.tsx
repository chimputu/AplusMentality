...
import { ArrowLeft, CheckCircle, Clock, Award, FileQuestion, User, Calendar, MessageCircle } from 'lucide-react';
...
export default async function StudentQuizResultsPage({ params }: PageProps) {
  ...
  if (!quizData || !quizData.isPublished) {
    notFound();
  }
  ...
  const quiz = {
    ...
    dueDate: quizData.dueDate ? quizData.dueDate.toISOString() : null,
    submissions: quizData.submissions.map((s: any) => ({
      ...s,
  ...
  // If no submission, redirect to the quiz page
  if (!submission) {
    redirect(`/student/quizzes/${id}`);
  }

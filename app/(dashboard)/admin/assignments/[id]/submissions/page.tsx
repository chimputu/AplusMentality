// app/(dashboard)/admin/assignments/[id]/submissions/page.tsx
'use client';
...
export default function AdminSubmissionsPage() {
  ...
  const openGradeModal = (sub: Submission) => {
    ...
    setTotalInput(String(sub.total ?? assignment?.maxScore ?? 100));
  ...
  const saveGrade = async () => {
    ...
    try {
      const res = await fetch(
        `/api/assignments/submissions/${grading.id}/grade`,
        {
          method: 'PUT',
  ...
  if (!assignment) return <div className="text-center py-8">Not found.</div>;
                    ...
                    <td className="px-4 py-3">
                      {sub.fileUrl ? (
                        ...
                        <span className="text-xs text-gray-500">
                          Form: {sub.responseId.slice(0, 12)}…
                        </span>
                      ) : (
                    ...
                    <td className="px-4 py-3">
                      {sub.score !== null ? (
                        <span className="font-semibold text-gray-900 dark:text-gray-100">
                          {sub.score}/{sub.total ?? assignment.maxScore ?? '—'}
                        </span>
                      ) : (
                        <span className="text-gray-400">—</span>
                      )}

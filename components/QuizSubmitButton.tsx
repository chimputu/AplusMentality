...
export default function QuizSubmitButton({ quizId, formUrl, isGoogleForm }: QuizSubmitButtonProps) {
  ...
  const handleSubmit = async () => {
    ...
    if (!confirm('Have you completed the quiz? Click OK to submit.')) return;
    ...
    try {
      ...
      if (res.ok) {
        if (isGoogleForm && formUrl) {
          window.location.href = formUrl; // redirect to Google Forms score page
        } else {
          router.push('/student/quizzes?submitted=true');
          router.refresh();
        }
      } else {
